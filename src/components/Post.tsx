...
const PostList = ({ allPosts }: { allPosts?: boolean }) => {
  ...
  return (
    <>
      <h1 className='my-3 mx-3 text-2xl uppercase font-black'>Post</h1>
      <ul>
        {posts.map(post => (
          <li className='mt-6 mx-3' key={post.id}>
      ...
      {!allPosts && (
        <div className="flex justify-center items-center mt-4">
          ...
          {Array.from({ length: totalPages }, (_, index) => (
            <button
              key={index}
              className={`mx-1 px-3 py-1 rounded ${currentPage === index + 1 ? 'bg-blue-700 text-white' : 'bg-blue-200 text-blue-700 hover:bg-blue-300 hover:text-blue-800'}`}
